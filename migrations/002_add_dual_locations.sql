...
ADD COLUMN IF NOT EXISTS storage_location_id INTEGER REFERENCES locations(id) ON DELETE SET NULL;

-- Add shelf location (display/sales area - usually retail floor/shelf)
ALTER TABLE products 
ADD COLUMN IF NOT EXISTS shelf_location_id INTEGER REFERENCES locations(id) ON DELETE SET NULL;
